...
void shady_close_animation_begin(struct shady_server*s){struct shady_toplevel*t=NULL;if(!wl_list_empty(&s->toplevels))t=wl_container_of(s->toplevels.next,t,link);if(!t)return;if(!s->config.close_animation){wlr_xdg_toplevel_send_close(t->xdg_toplevel);return;}if(t->close.state!=SHADY_CLOSE_IDLE&&t->close.state!=SHADY_CLOSE_ARMED)return;t->close.state=SHADY_CLOSE_CRUMPLING;t->close.progress=0;t->close.wait_time=0;shady_window_motion_add_impulse(s,t,.10f,-.07f,0.f,0.f);shady_render_schedule_all_outputs(s);}
