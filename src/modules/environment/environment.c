...
bool shady_environment_load_world(struct shady_server*s){
	if(!s->config.environment_obj||!s->config.environment_obj_path[0])return true;
	struct shady_mesh mesh;
	if(!shady_obj_load(s->config.environment_obj_path,&mesh))return false;
	struct shady_box_collider box={
		mesh.bounds_min[0],mesh.bounds_max[0],
		mesh.bounds_min[1],mesh.bounds_max[1],
		mesh.bounds_min[2],mesh.bounds_max[2]
	};
	bool added=shady_world_add_collider(&s->world,box);
	if(added)wlr_log(WLR_INFO,
		"environment: registered OBJ bounds collider %.3f %.3f %.3f -> %.3f %.3f %.3f",
		box.min_x,box.min_y,box.min_z,box.max_x,box.max_y,box.max_z);
	else wlr_log(WLR_ERROR,"environment: world collider capacity reached");
